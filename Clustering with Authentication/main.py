...
def run_experiment(X):
    ...
    for i in range(2,11):
        ...
        k_silhouttes.append(silhouette_score(X,kmeans.labels_))
        agglo_silhouttes.append(silhouette_score(X, agglo.labels_))
        
    
    metrics_df = pd.DataFrame({'n_clusters':np.arange(2,11),
                               'KMeans': k_silhouttes,
                              'Agglo': agglo_silhouttes})
...
if st.session_state['authentication_status']:
    ...
    if username == 'marketing':
        ...
        st.title("Content for Marketing Team")
        display_marketing_content()
    
    elif username =='datascience':
        st.write(f'Welcome {st.session_state["name"]}')
        st.title("Content for Data Team")
        display_ds_content()       
